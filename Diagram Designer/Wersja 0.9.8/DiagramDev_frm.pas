...
type
  TDiagramDev = class(TForm)
    ...
    procedure pnl_wzorMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure pnl_wzorMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure pnl_wzorMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    procedure btn_nowyClick(Sender: TObject);
    procedure Zmienazw1Click(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure Czysc_tablice;

    procedure Rysuj_powiazanie(od_obiektu, do_obiektu : Integer; od_strzalka, do_strzalka : Boolean);
    procedure Rysuj_powiazania;
    function Ile_powiazan(id_obiektu: Integer): Integer;
    function Ktore_to_powiazanie(id_obiektu, id_obiektu_od, id_obiektu_do: Integer): Integer;
 ...
 for i:= 1 to max_obiektow do
  Begin
   if Assigned(diagram[i].panel) then
    Begin
     if diagram[i].panel.Caption=nazwa_statusu then wynik:=diagram[i].id_obiektu;
    End;
  End;
 ID_obiektu_z_nazwy:=wynik;
End;

function TDiagramDev.Ktore_to_powiazanie(id_obiektu, id_obiektu_od, id_obiektu_do: Integer): Integer;
var
  i: Integer;
  wynik: Integer;
  koniec : Boolean;
 ...
 for i := 1  to max_powiazan do
  Begin
   if koniec=False then
    Begin
     if (Powiazania[i].od_obiektu=id_obiektu) OR ((Powiazania[i].do_obiektu=id_obiektu)) then wynik:=wynik+1;
     if (Powiazania[i].od_obiektu=id_obiektu_od) and (Powiazania[i].do_obiektu=id_obiektu_do) then koniec:=True;
    End;
  End;
...
Begin
 ...
 for i := 1  to max_powiazan do
  Begin
   if (Powiazania[i].do_obiektu=id_obiektu) or (Powiazania[i].od_obiektu=id_obiektu) then wynik:=wynik+1;
  End;
 Ile_powiazan:=wynik;
End;
   ...
   if Powiazania[i].od_obiektu>0 then
    Begin
     for o := 1 to max_obiektow do
      Begin
       if diagram[o].id_obiektu=Powiazania[i].od_obiektu then pnl_od:=o;
      End;
     for o := 1 to max_obiektow do
      Begin
       if diagram[o].id_obiektu=Powiazania[i].do_obiektu then pnl_do:=o;
      End;

     Rysuj_powiazanie(pnl_od, pnl_do, Powiazania[i].od_strzalka, Powiazania[i].do_strzalka);
    End;
  End;
End;

procedure TDiagramDev.Rysuj_powiazanie(od_obiektu, do_obiektu : Integer; od_strzalka, do_strzalka : Boolean);
var
...
if (od_strzalka) or (do_strzalka) then
 ...
 ktore_od:=Ktore_to_powiazanie(Diagram[od_obiektu].id_obiektu, Diagram[od_obiektu].id_obiektu, Diagram[do_obiektu].id_obiektu);
 ktore_do:=Ktore_to_powiazanie(Diagram[do_obiektu].id_obiektu, Diagram[od_obiektu].id_obiektu, Diagram[do_obiektu].id_obiektu);

 Canvas.Pen.Style := psSolid;

 if wybrany_panel>0 then
  Begin
   if (od_obiektu=wybrany_panel) or ((do_obiektu=wybrany_panel) and (od_strzalka)) then
    Begin
 ...
 if do_panelu.Height>od_panelu.Height then max_wys_analizy:=do_panelu.Height;

 roznica:=ABS(od_panelu.Top-do_panelu.Top);
 if roznica<(max_wys_analizy+(HeadLength*2)) then lamanie:=3 else lamanie:=2;
 ...
 if lamanie=2 then
  ...
  if pion='G' then
   Begin
    ...
    ox1:=x1; oy1:=y1-odlegloscY;
    Canvas.LineTo(x2, y1-odlegloscY);
    ox2:=x2; oy2:=y1-odlegloscY;
    Canvas.LineTo(x2, y2);
   End;
  if pion='D' then
   Begin
  ...
  Begin
   if pion='G' then
    Begin
     ...
     ox1:=x1; oy1:=y2-(HeadLength*2);
     Canvas.LineTo(x2, y2-(HeadLength*2));
     ox2:=x2; oy2:=y2-(HeadLength*2);
     Canvas.LineTo(x2, y2);
    End;
   if pion='D' then
    Begin
...
begin
  ...
  Begin
   if Assigned(diagram[o].panel) then
    Begin
     ...
     for i := 1 to max_powiazan do
      Begin
       if (Powiazania[i].od_obiektu=id_obiektu) or (Powiazania[i].do_obiektu=id_obiektu) then czy_znaleziono:=True;
      End;

     if czy_znaleziono then diagram[o].panel.Color:=pnl_wzor.Color
     else
      Begin
 ...
 if czy_znaleziono_niepowiazane=True then ShowMessage('UWAGA!!!'+#13+'Znaleziono i oznaczono obiekty niepowi¹zane!')
 else ShowMessage('Struktura spójna!'+#13+'Wszystko OK!');
end;
...
begin
 ...
 for i := 1 to max_powiazan do
  Begin
   if (Powiazania[i].od_obiektu=index_obiektu) or (Powiazania[i].do_obiektu=index_obiektu) then
    Begin
     Powiazania[i].od_obiektu:=0;
     Powiazania[i].do_obiektu:=0;
     Powiazania[i].od_strzalka:=False;
...
begin
 ...
 for i := 1 to max_obiektow do
  Begin
   if Assigned(diagram[i].panel) then
    Begin
     if diagram[i].panel.Caption=status then index_obiektu:=diagram[i].id_obiektu;
    End;
  End;
  ...
  Begin
   ...
   for i := 1 to max_obiektow do
    Begin
     if Assigned(diagram[i].panel) then
      Begin
       if (panel<>diagram[i].panel) and (diagram[i].panel.Caption=nowa_nazwa) then czy_ok:=False;
      End;
    End;

   if czy_ok then panel.Caption:=nowa_nazwa
   else ShowMessage('Podany status ju¿ istnieje!'+#13+'Obiekt zosta³ dodany, ale nale¿y przydzieliæ mu niepowtarzalny status!');
  End;
...
begin
 ...
 if CarbonStyle1.Checked then TStyleManager.TrySetStyle('Carbon')
  ...
  Begin
   ...
   for i := 1 to max_obiektow do
    Begin
     if Assigned(diagram[i].panel) then
      Begin
       if diagram[i].panel.Caption<>status then DiagramPowiazanie.cbox_nazwy.Items.Add(diagram[i].panel.Caption);
      End;
    End;
   DiagramPowiazanie.cbox_od.Checked:=False;
   DiagramPowiazanie.cbox_do.Checked:=True;
   DiagramPowiazanie.Show;
   if DiagramPowiazanie.cbox_nazwy.Items.Count=1 then DiagramPowiazanie.cbox_nazwy.ItemIndex:=0;
  End
 else ShowMessage('Obiekt musi mieæ zdefiniowan¹ nazwê!');
end;
...
procedure TDiagramDev.pnl_wzorMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
Var
 status : String;
begin
...
procedure TDiagramDev.pnl_wzorMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
const
 minWidth = 20;
 minHeight = 20;
var
...
procedure TDiagramDev.pnl_wzorMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
 if inReposition then
  begin
   ...
   for i := 0 to plik.Count-1 do
    Begin
     ...
     if linia='[OBIEKT_BEGIN]' then
      Begin
       id:=Trim(StringReplace(plik[i+1],'ID=','',[rfReplaceAll,rfIgnoreCase]));
       x:=Trim(StringReplace(plik[i+2],'X=','',[rfReplaceAll,rfIgnoreCase]));
       y:=Trim(StringReplace(plik[i+3],'Y=','',[rfReplaceAll,rfIgnoreCase]));
       w:=Trim(StringReplace(plik[i+4],'W=','',[rfReplaceAll,rfIgnoreCase]));
       h:=Trim(StringReplace(plik[i+5],'H=','',[rfReplaceAll,rfIgnoreCase]));
       text:=Trim(StringReplace(plik[i+6],'TEXT=','',[rfReplaceAll,rfIgnoreCase]));
     ...
     if linia='[POWIAZANIE_BEGIN]' then
      Begin
       od_id:=Trim(StringReplace(plik[i+1],'OD=','',[rfReplaceAll,rfIgnoreCase]));
       do_id:=Trim(StringReplace(plik[i+2],'DO=','',[rfReplaceAll,rfIgnoreCase]));
       sod:=Trim(StringReplace(plik[i+3],'SOD=','',[rfReplaceAll,rfIgnoreCase]));
       sdo:=Trim(StringReplace(plik[i+4],'SDO=','',[rfReplaceAll,rfIgnoreCase]));
  ...
  Begin
   ...
   if FileExists(SaveDialog.FileName) then
    Begin
     ...
     PWideChar('Czy na pewno chcesz zast¹piæ wskazany plik?' + #13 +'Istniej¹cy diagram zostanie nadpisany!!!!'), 'Zast¹piæ plik diagramu?', MB_YESNO + MB_ICONQUESTION);
     if wyb = mrYes then czy_zapisac:=True;
    End
   else czy_zapisac:=True;
   ...
   Rewrite(plik);
    ...
    for i := 1 to max_powiazan do
     ...
     End;
   CloseFile(plik);
   ShowMessage('Diagram poprawnie zapisano do pliku:'+#13+SaveDialog.FileName);
   End;
  End;
end;
...
var
  ...
  i: Integer;
begin
 nowa_nazwa:=Trim(InputBox('Nazwa statusu','Status: ',(PopupMenuPanelu.PopupComponent as TPanel).Caption));
 czy_ok:=True;
 ...
 for i := 1 to max_obiektow do
  Begin
   if Assigned(diagram[i].panel) then
    Begin
     if ((PopupMenuPanelu.PopupComponent as TPanel)<>diagram[i].panel) and (diagram[i].panel.Caption=nowa_nazwa) then czy_ok:=False;
    End;
  End;

 if czy_ok then (PopupMenuPanelu.PopupComponent as TPanel).Caption:=nowa_nazwa
 else ShowMessage('Podany status ju¿ istnieje!');
end;
